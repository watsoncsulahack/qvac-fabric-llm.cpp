...
// l2_norm
//------------------------------------------------------------------------------
// y = x * (1.0 / max(sqrt(sum(x_i^2)), eps))
// Mirrors ggml_compute_forward_l2_norm_f32 in ggml-cpu.
// This kernel depends on subgroup size (matches kernel_rms_norm above).
#ifdef INTEL_GPU
...
kernel void kernel_l2_norm(
    ...
    if (get_local_id(0) == 0) {
        // remaining tail elements (squared)
    ...
    const float sum_sq = sum[0];
    const float scale  = 1.0f / fmax(sqrt(sum_sq), eps);
