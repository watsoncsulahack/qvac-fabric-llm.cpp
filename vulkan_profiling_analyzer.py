...
class ProfilingSection:
    ...
    mat_mul_summaries: Dict[str, Tuple[int, float]] = field(default_factory=dict)  # operation -> (total_ops, avg_time)
...
class GlobalStats:
    ...
    mat_mul_normalized: Dict[str, Dict[str, float]] = field(default_factory=dict)  # normalized by tensor size
...
class VulkanProfilingAnalyzer:
    ...
    def parse_log_file(self) -> None:
        ...
        print(f"Found {len(inference_step_contents)} inference steps")
    ...
    def _parse_device_allocation(self) -> DeviceAllocation:
        ...
        # Count GPU layers
        device_alloc.gpu_layers = len(re.findall(r'assigned to device Vulkan\d+', content))
        ...
        for op_type in cpu_ops:
            ...
            device_alloc.cpu_operation_types[op_type] = device_alloc.cpu_operation_types.get(op_type, 0) + 1
    ...
    def _parse_model_info(self) -> ModelInfo:
        ...
        model_file_match = re.search(r'loaded meta data with [^/]+ from ([^\s]+)', content)
        if model_file_match:
        ...
        for field, pattern in kv_patterns.items():
            ...
            if match:
                if field in ['layer_count', 'context_length', 'embedding_length', 'feed_forward_length', 'attention_heads', 'attention_heads_kv']:
                    setattr(model_info, field, int(match.group(2) if len(match.groups()) > 1 else match.group(1)))
                else:
                    setattr(model_info, field, match.group(1).strip())
        ...
        tensor_types = re.findall(r'llama_model_loader: - type\s+(\w+):\s+(\d+) tensors', content)
    ...
    def _parse_section(self, section_id: int, content: str) -> None:
        ...
        general_match = re.search(r'Legacy Timing Summary:\n-+\n(.*?)\n(?=Enhanced Triplet|$)', content, re.DOTALL)
        if general_match:
            section.general_timings = self._parse_general_timings(general_match.group(1))
        
        # Parse Enhanced Triplet Timing Analysis
        mat_mul_match = re.search(r'Enhanced Triplet Timing Analysis by Operation Type:\n=+\n(.*?)\nOverall Statistics:', content, re.DOTALL)
        if mat_mul_match:
            section.mat_mul_timings, section.mat_mul_summaries = self._parse_mat_mul_timings(mat_mul_match.group(1))
        
        # Parse overall statistics
        stats_match = re.search(r'Total operation types: (\d+)\nTotal variations: (\d+)', content)
        if stats_match:
    ...
    def _parse_general_timings(self, content: str) -> List[GeneralTimingEntry]:
        ...
        for line in lines:
            ...
            if ':' in line:
                ...
                if ' ' in operation_part and any(c in operation_part for c in ['=', 'x']):
                    # Has parameters
                ...
                if timing_match:
                    ...
                    timings.append(GeneralTimingEntry(
                        operation=operation,
                        count=count,
    ...
    def _parse_mat_mul_timings(self, content: str) -> Tuple[List[MatMulTimingEntry], Dict[str, Tuple[int, float]]]:
                ...
                if i + 1 < len(operation_sections):
                    ...
                    for line in lines:
                        ...
                        if line.startswith('→'):
                            ...
                            summary_match = re.search(r'(\w+)\s+Summary:\s+(\d+)\s+total ops,\s+([\d.]+)\s+us avg', line)
                            if summary_match:
                            ...
                            entry_match = re.match(r'(.+?)\s*\|\s*(.+?):\s*(\d+)\s+ops,\s*([\d.]+)\s+us avg', line)
                            if entry_match:
                                ...
                                timings.append(MatMulTimingEntry(
    ...
    def compute_global_statistics(self) -> GlobalStats:
        ...
        for section in self.inference_steps:
            for timing in section.general_timings:
                ...
                key = f"{timing.operation}({timing.params})" if timing.params else timing.operation
        ...
        mat_mul_data = defaultdict(list)  # (operation_type, data_types, matrix_dims) -> list of (count, avg_time_us)
        
        for section in self.inference_steps:
            for timing in section.mat_mul_timings:
                key = f"{timing.operation_type}|{timing.data_types}|{timing.matrix_dims}"
    ...
    def _calculate_normalized_mat_mul_performance(self, global_stats: GlobalStats) -> None:
        """Calculate MatMul performance normalized by tensor sizes."""
        # Group operations by operation type and tensor types
        grouped_operations = defaultdict(list)  # (operation_type, tensor_types) -> list of normalized times
        ...
        for operation_key, stats in global_stats.mat_mul_stats.items():
            ...
            if match:
                ...
                dims_match = re.search(r'\[(\d+)x(\d+)\]\s*x\s*\[(\d+)x(\d+)\]\s*->\s*\[(\d+)x(\d+)\]', operation_key)
                if dims_match:
                    # Extract input tensor dimensions
                    a = int(dims_match.group(1))  # rows of first matrix
                    b = int(dims_match.group(2))  # cols of first matrix / rows of second matrix
                    ...
                    normalized_time = stats['avg_time_mean'] / computational_volume if computational_volume > 0 else 0
                    
                    # Group key: operation type + tensor types
                    group_key = f"{operation_type} {tensor_types}"
                    grouped_operations[group_key].append((normalized_time, stats['total_operations']))
        ...
        for group_key, data_points in grouped_operations.items():
            if data_points:
                ...
                weighted_sum = sum(norm_time * weight for norm_time, weight in zip(normalized_times, weights))
                total_weight = sum(weights)
                
                weighted_mean_normalized_time = weighted_sum / total_weight if total_weight > 0 else 0
    ...
    def _create_normalized_time_plot(self, global_stats: GlobalStats, output_dir: str = ".", return_base64: bool = False) -> str:
        ...
        try:
            ...
            sorted_data = sorted(global_stats.mat_mul_normalized.items(),
                               key=lambda x: x[1]['mean_normalized_time'])
            
            for group_key, stats in sorted_data:
                # Shorten operation names for better readability
                short_name = group_key.replace('ggml_vk_mul_mat_vec_', 'vec_').replace('ggml_vk_mul_mat_', 'mat_')
                short_name = short_name.replace('_q_f16', '').replace('_f16_f32', '').replace('_nc_f16_f32', '_nc').replace('_p021_f16_f32', '_p021')
                operations.append(short_name)
                normalized_times.append(stats['mean_normalized_time'])
            
            # Create the plot
            plt.figure(figsize=(12, 8))
            bars = plt.bar(range(len(operations)), normalized_times, color='steelblue', alpha=0.7)
            
            # Customize the plot
            plt.title('MatMul Operations: Normalized Time Efficiency\n(Lower = More Efficient)', fontsize=14, fontweight='bold')
    ...
    def generate_report(self, global_stats: GlobalStats) -> str:
        ...
        if global_stats.model_info.embedding_length > 0:
            report.append(f"Embedding Dimension: {global_stats.model_info.embedding_length:,}")
        if global_stats.model_info.attention_heads > 0:
            if global_stats.model_info.attention_heads_kv > 0 and global_stats.model_info.attention_heads_kv != global_stats.model_info.attention_heads:
                report.append(f"Attention Heads: {global_stats.model_info.attention_heads} ({global_stats.model_info.attention_heads_kv} KV heads)")
            else:
                report.append(f"Attention Heads: {global_stats.model_info.attention_heads}")
        ...
        if global_stats.model_info.tensor_types:
            ...
            for tensor_type, count in sorted(global_stats.model_info.tensor_types.items(), key=lambda x: x[1], reverse=True):
        ...
        report.append(f"GPU Operations: {global_stats.device_allocation.gpu_operations:,}")
        report.append(f"CPU Operations: {global_stats.device_allocation.cpu_operations:,}")
        total_ops_device = global_stats.device_allocation.gpu_operations + global_stats.device_allocation.cpu_operations
        gpu_percentage = (global_stats.device_allocation.gpu_operations / total_ops_device * 100) if total_ops_device > 0 else 0
        cpu_percentage = (global_stats.device_allocation.cpu_operations / total_ops_device * 100) if total_ops_device > 0 else 0
        report.append(f"GPU Utilization: {gpu_percentage:.1f}% ({global_stats.device_allocation.gpu_operations:,}/{total_ops_device:,} operations)")
        report.append(f"CPU Utilization: {cpu_percentage:.1f}% ({global_stats.device_allocation.cpu_operations:,}/{total_ops_device:,} operations)")
        
        # Show CPU operation types if any
        if global_stats.device_allocation.cpu_operation_types:
            report.append("CPU Operations by Type:")
            for op_type, count in sorted(global_stats.device_allocation.cpu_operation_types.items(), key=lambda x: x[1], reverse=True):
                report.append(f"  {op_type}: {count:,} operations")
        
        report.append("")
        
        # Inference Steps Summary
        report.append(f"Total inference steps analyzed: {global_stats.total_inference_steps}")
        report.append(f"Note: Device allocation counts ALL executed operations ({total_ops_device:,}),")
        report.append(f"      while timing statistics below only count profiled operations.")
        report.append("")
        ...
        sorted_general = sorted(global_stats.general_stats.items(), 
                              key=lambda x: x[1]['avg_time_mean'], reverse=True)
        ...
        for operation, stats in sorted_general:
            ...
            report.append(f"  Appeared in {stats['total_inference_steps']}/{global_stats.total_inference_steps} inference steps")
            report.append(f"  Average Time (μs): {stats['avg_time_mean']:.2f} ± {stats['avg_time_stdev']:.2f}")
            report.append(f"    Min: {stats['avg_time_min']:.2f}, Max: {stats['avg_time_max']:.2f}, Median: {stats['avg_time_median']:.2f}")
            report.append(f"  Operations per Inference Step: {stats['count_mean']:.1f} ± {stats['count_stdev']:.1f}" if stats.get('count_stdev') else f"  Operations per Inference Step: {stats['count_mean']:.1f}")
            report.append("")
        ...
        # Sort by average time (highest first)
        sorted_mat_mul = sorted(global_stats.mat_mul_stats.items(),
                                key=lambda x: x[1]['avg_time_mean'], reverse=True)
        ...
        if global_stats.mat_mul_normalized:
            ...
            report.append("(Weighted mean normalized time by operation type and tensor types)")
            report.append("=" * 60)
            report.append("")
            
            # Sort by mean normalized time (most efficient first - lowest time per operation)
            sorted_normalized = sorted(global_stats.mat_mul_normalized.items(),
                                     key=lambda x: x[1]['mean_normalized_time'])
            
            for group_key, stats in sorted_normalized:
                report.append(f"Operation: {group_key}")
                report.append(f"  Weighted Mean Normalized Time: {stats['mean_normalized_time']:.9f} μs per tensor element")
                report.append(f"  (Based on {stats['total_operations']:,} total operations across {stats['sample_count']} matrix size variations)")
                report.append("")
        ...
        for operation, stats in global_stats.general_stats.items():
            ...
            total_times.append((operation, total_time, stats['total_operations'], stats['avg_time_mean']))
        ...
        for i, (operation, total_time, total_ops, avg_time) in enumerate(total_times[:20]):
            report.append(f"Operation: {operation}")
            report.append(f"  Total Operations: {total_ops:,}")
            report.append(f"  Total Execution Time: {total_time:,.2f} μs ({total_time/1000:.2f} ms)")
        ...
        total_general_ops = sum(stats['total_operations'] for stats in global_stats.general_stats.values())
        total_mat_mul_ops = sum(stats['total_operations'] for stats in global_stats.mat_mul_stats.values())
        total_all_ops = total_general_ops + total_mat_mul_ops
        
        # Calculate percentages
        general_percentage = (total_general_ops / total_all_ops * 100) if total_all_ops > 0 else 0
        mat_mul_percentage = (total_mat_mul_ops / total_all_ops * 100) if total_all_ops > 0 else 0
        
        # Calculate total execution times
        total_general_time = sum(stats['total_operations'] * stats['avg_time_mean'] for stats in global_stats.general_stats.values())
        total_mat_mul_time = sum(stats['total_operations'] * stats['avg_time_mean'] for stats in global_stats.mat_mul_stats.values())
        total_execution_time = total_general_time + total_mat_mul_time
        
        # Calculate time per inference step
        time_per_inference_step = total_execution_time / global_stats.total_inference_steps if global_stats.total_inference_steps > 0 else 0
        
        report.append(f"Total General Operations: {total_general_ops:,} ({general_percentage:.1f}%)")
        report.append(f"Total MatMul Operations: {total_mat_mul_ops:,} ({mat_mul_percentage:.1f}%)")
        report.append(f"Total Profiled Operations: {total_all_ops:,}")
        report.append(f"Non-profiled Operations: {total_ops_device - total_all_ops:,} (setup, memory management, etc.)")
        report.append("")
        report.append(f"Total Execution Time: {total_execution_time:,.2f} μs ({total_execution_time/1000:.2f} ms)")
        report.append(f"  General Operations: {total_general_time:,.2f} μs ({total_general_time/total_execution_time*100:.1f}%)")
        report.append(f"  MatMul Operations: {total_mat_mul_time:,.2f} μs ({total_mat_mul_time/total_execution_time*100:.1f}%)")
        report.append(f"Average Time per Inference Step: {time_per_inference_step:,.2f} μs ({time_per_inference_step/1000:.2f} ms)")
        report.append("")
        report.append(f"Unique General Operation Types: {len(global_stats.general_stats)}")
        report.append(f"Unique MatMul Operation Types: {len(global_stats.mat_mul_stats)}")
        
        return "\n".join(report)
    
    def generate_markdown_report(self, global_stats: GlobalStats, output_dir: str = ".") -> str:
        ...
        report.append(f"- **GPU Device:** {global_stats.device_allocation.gpu_device}")
        report.append(f"- **GPU Layers:** {global_stats.device_allocation.gpu_layers}")
        report.append(f"- **GPU Operations:** {global_stats.device_allocation.gpu_operations:,} ({gpu_percentage:.1f}%)")
        report.append(f"- **CPU Operations:** {global_stats.device_allocation.cpu_operations:,} ({cpu_percentage:.1f}%)")
        ...
        for operation, stats in sorted_general:
            ...
            report.append(f"- **Total Operations:** {stats['total_operations']:,}")
            report.append(f"- **Appeared in:** {stats['total_inference_steps']}/{global_stats.total_inference_steps} inference steps")
            report.append(f"- **Average Time:** {stats['avg_time_mean']:.2f} ± {stats['avg_time_stdev']:.2f} μs")
            report.append(f"- **Min/Max:** {stats['avg_time_min']:.2f} / {stats['avg_time_max']:.2f} μs, Median: {stats['avg_time_median']:.2f} μs")
            report.append(f"- **Operations per Inference Step:** {stats['count_mean']:.1f}")
            report.append("")
        ...
        if global_stats.mat_mul_normalized:
            ...
            if plot_filename and os.path.exists(plot_filename):
                ...
                report.append("")
                report.append("*Chart shows weighted mean normalized time for each operation type and tensor type combination. Lower values indicate higher efficiency.*")
                report.append("")
    ...
    def convert_markdown_to_html(self, markdown_content: str, html_filename: str, global_stats: GlobalStats) -> bool:
        """Convert markdown content to HTML with base64-embedded images."""
        try:
            # Generate base64-encoded plot
            plot_base64 = self._create_normalized_time_plot(global_stats, return_base64=True)
            ...
            html_content = re.sub(r'^  - (General Operations:.*)$', r'<li style="margin-left: 20px;">\1</li>', html_content, flags=re.MULTILINE)
            html_content = re.sub(r'^  - (MatMul Operations:.*)$', r'<li style="margin-left: 20px;">\1</li>', html_content, flags=re.MULTILINE)
            ...
            html_content = re.sub(r'(<h[1-6][^>]*>.*?</h[1-6]>)<br>', r'\1', html_content)
...
def main():
    parser = argparse.ArgumentParser(description='Analyze Vulkan profiling results from log file')
    parser.add_argument('log_file', nargs='?', default='log.txt',
                       help='Path to log file (default: log.txt)')
    parser.add_argument('--report-output', help='Save report to specified file (default: print to stdout)')
    parser.add_argument('--markdown-output', help='Save markdown report to specified file')
    parser.add_argument('--html-output', help='Save HTML report to specified file (converted from markdown with embedded images)')
    ...
    try:
        ...
        if args.markdown_output:
            markdown_dir = os.path.dirname(os.path.abspath(args.markdown_output)) if os.path.dirname(args.markdown_output) else "."
            markdown_report = analyzer.generate_markdown_report(global_stats, markdown_dir)
            with open(args.markdown_output, 'w') as f:
                f.write(markdown_report)
            print(f"Markdown report saved to: {args.markdown_output}")
        
        if args.html_output:
            html_dir = os.path.dirname(os.path.abspath(args.html_output)) if os.path.dirname(args.html_output) else "."
            markdown_report = analyzer.generate_markdown_report(global_stats, html_dir)
            success = analyzer.convert_markdown_to_html(markdown_report, args.html_output, global_stats)
            if success:
                print(f"HTML report saved to: {args.html_output} (self-contained with embedded images)")
            else:
                print("HTML generation failed. See error messages above.")
